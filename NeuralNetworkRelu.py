...
f=gzip.open("ann_MNIST_edges_dynamic_batch_leaky_relu2_3.pkl")
(network1.weights,network2.weights,network3.weights)=cPickle.load(f)
f.close()
...
fileEpoch=open("Relus_epoch2.txt","w+")
for i in range(iterations):
    ...
    for j in range(len(train_set[0])):
        ...
        inputLayer.outPut=np.array(np.array(train_set[0][j]).flatten())
        ...
        network4.backProp(learningRate,label)
        network3.backProp(learningRate)
        network2.backProp(learningRate)
        network1.backProp(learningRate)
        batchNum+=1

    print str(i)+" th iteration"+" TP= "+str(TP_new)
    if i+1%2==0:
        if TP_new-oldTp<0.0001:
            if learningRate>=0.000002:
                learningRate/=2
                print learningRate
                oldTp=TP_new
    f = gzip.open("ann_MNIST_edges_dynamic_batch_leaky_relu2_4.pkl", 'wb')
    cPickle.dump((network1.weights,network2.weights,network3.weights,network4.weights), f)
    f.close()
    endtime=time.time()
    fileEpoch.write("epoch "+str(i+1)+" accuracy "+str(TP_new)+"time= "+str(endtime-starttime)+"\n")
fileEpoch.close()
